...
### Note
```powershell
Stop-Process -Name rust-flashcards -Force
...
cargo run                   # build + run the default deck named "deck" with "Default Display Name" as display name
cargo run -- -n Zoubida     # build + run the default deck named "deck" with "Zoubida" as display name
cargo run -- --deck-display-name "Léon et les Guépards"

cargo run -- --deck-id test_42 # build + run the deck named "test_42" with "Default Display Name" as display name
cargo run -- --deck-id test_42 -n Marcel # build + run the deck named "test_42" with "Marcel" as display name

# Rebuild specific deck
cargo run -- --rebuild-deck-id test -r test

# Load specific deck with custom name
cargo run -- --deck-id rust --deck-display-name "Rust Flashcards"
